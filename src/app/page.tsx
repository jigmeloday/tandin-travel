...
export default function Home() {
	...
	const bestSellingPackages = flagshipSection.packageIds
	...
	const gridPackages = featuredPackages.grid
		.map((id) => getPackageById(id))
		.filter((pkg): pkg is PackageCard => pkg !== undefined);
	...
	// Get tour types
	const tourTypeItems = tourTypes
		.map((id) => TOUR_TYPES.find((t) => t.id === id))
		.filter((tour): tour is TourType => tour !== undefined);
	...
	return (
		<main>
			...
			<section className="flex flex-col items-center justify-center px-[16px] lg:px-[32px] my-[90px]">
				...
				<div className="flex flex-col items-center text-center">
					...
					<div className="lg:w-[920px]">
						<p className="text-center my-[24px]">{introduction.description}</p>
					</div>
					<div className="lg:min-w-[250px]">
						<span className="font-bold">{introduction.tagline}</span>
					</div>
				</div>
			...
			<section className="flex flex-col items-center justify-center my-[90px] lg:px-[32px] px-[16px]">
				...
				<p className="font-bold mt-2">{blogSection.subtitle}</p>
